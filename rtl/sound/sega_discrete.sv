...
//
//  The $3E/$3F latch map follows the Sega schematics and MAME netlists. Voice
//  synthesis is behavioral; documented oscillator and mixer values come from
...
module sega_discrete #(
	...
	logic [24:0] acc;
	logic        ce;
	wire [24:0]  acc_next = acc + SAMPLE_HZ;
	always_ff @(posedge clk) begin
		if (reset) begin acc <= '0; ce <= 1'b0; end
		else if (acc_next >= MASTER_HZ) begin
			acc <= acc_next - MASTER_HZ; ce <= 1'b1;
		end else begin
			acc <= acc_next;             ce <= 1'b0;
		end
	end
	...
	// ------------------------------------------------------------------
	// The two sound latches store active-low controls.
	// ------------------------------------------------------------------
	logic [7:0] lo, hi;
	always_ff @(posedge clk) begin
	...
	// Idle is $FF; pulling a bit low triggers its voice.
	wire [7:0] lo_g = ~lo;
	wire [7:0] hi_g = ~hi;

	wire is_sf    = (game == GAME_SPACFURY);
	wire is_elim  = (game == GAME_ELIM2) || (game == GAME_ELIM2C) ||
	                (game == GAME_ELIM4);
	...
	//       D4 SKITTER      D5 ENEMY_SHIP   D6 HEXAGONS_LSB
	//       D7 HEXAGONS_MSB
	// ==================================================================
	...
	// Leave headroom for simultaneous voices.
