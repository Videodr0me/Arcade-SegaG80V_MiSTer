...
//  Interface behavior follows the 8253 use in MAME segausb.cpp.
//
//  Implements modes 1 and 3, the only modes used by the current firmware.
//
//  Register interface, written through the 8035's work RAM window:
//    offset 0-2   count byte for that channel, per its latch mode
//    offset 3     control: {channel[1:0], latchmode[1:0], clockmode[2:0], bcd (ignored)}
//
...
module usb_timer (
	...
	// A coincident channel clock observes the newly written state.
	integer i;
