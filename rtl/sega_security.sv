...
//
//  The chips permute the low address byte of RAM writes performed by opcode
//  $32 (LD (nnnn),A). Two bits from that opcode's PC select the permutation.
//
//  Cross-checked against MAME segag80_m.cpp and the Sega hardware mappings.
//
...
// B, C and D each invert one bit. Every term lands on a distinct output bit.
// ---------------------------------------------------------------------------
...
//
// Every chip selects on PC bit 0 and one higher bit. Masks 0x03, 0x09, and
// 0x11 select bits 1, 3, and 4. Chip-specific permutations are packed as
// {perm[3], perm[2], perm[1], perm[0]} and indexed by {pc[hibit], pc[0]}.
// ---------------------------------------------------------------------------
...
// Bus-side wrapper.
//
// Opcode $32 arms the scrambler. Its first RAM write consumes the armed state;
// any intervening opcode fetch disarms it.
// ---------------------------------------------------------------------------
