...
//  emit the beam path.
//
//  Cross-checked against MAME segag80v_v.cpp and the Sega X-Y schematics.
//  Chip designators refer to those drawings.
//
//  Output provides authentic beam samples and vector descriptors for the
//  doubled-density presentation DDA.
//
...
module sega_xy #(
	...
	//
	// One phase-generator slot lasts 16 VCL clocks. The U51/U50 timing drawing
	// leaves direct-VCL clocking unresolved; see docs/TECHNICAL_REFERENCE.md.
	...
	// Complete vector descriptor for the presentation-only shadow DDA.
	output logic        segment_start,
	output logic [11:0] segment_start_x,
	output logic [11:0] segment_start_y,
	output logic  [8:0] segment_duration,
	output logic  [7:0] segment_delta_x,
	output logic  [7:0] segment_delta_y,
	output logic        segment_x_negative,
	output logic        segment_y_negative,
	output logic  [5:0] segment_colour,
	output logic        segment_beam,
	output logic        segment_dot,
	...
	// shortest slot this sequencer can produce. It changes draw duration, not
	// list-budget accounting.
	localparam int DWELL    = (PHASE_CLKS < 3) ? 3 : PHASE_CLKS;
	// A symbol header costs 10 phases; each vector record costs four.
	...
	wire signed [31:0] remaining_ext = $signed({23'd0, remaining});
	wire [8:0] available_steps = (budget <= 0) ? 9'd0 :
	                             (budget < remaining_ext) ? budget[8:0] : remaining;
	...
	always_ff @(posedge clk) begin
		if (reset) begin
			...
			segment_delta_x <= '0;
			segment_delta_y <= '0;
			segment_x_negative <= 1'b0;
			segment_y_negative <= 1'b0;
			segment_colour <= '0;
			segment_beam <= 1'b0;
			segment_dot <= 1'b0;
			...
			if (frame_start) begin
				// EDGINT restarts the list and closes any unfinished frame.
				...
				// ---- sin/cos lookups and walk setup ------------------------
				// These three setup ticks are included in the vector-record cost.
				...
				ST_SIN_DONE: begin
					...
					segment_start <= 1'b1;
					segment_start_x <= curx;
					segment_start_y <= cury;
					segment_duration <= available_steps;
					segment_delta_x <= deltax;
					segment_delta_y <= sin_data;
					segment_x_negative <= sum_x[9];
					segment_y_negative <= sum_y[9];
					segment_colour <= attrib[6:1];
					segment_beam <= attrib[0] & (|attrib[6:1]);
					segment_dot <= (remaining == 9'd0);
					state    <= ST_STEP;
				end
				...
				ST_STEP: begin
					if (remaining == 9'd0) begin
						// Emit one sample when a vector starts with zero length.
						if (!dda_ran) begin
							out_x      <= cx[9:0];
							out_y      <= cy[9:0];
