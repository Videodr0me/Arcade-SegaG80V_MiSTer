...
module vfb_halo_pipeline #(
	...
	input  logic        color_space_g08_709,
	...
	localparam integer COLOR_PIPELINE_DELAY = 5;
	localparam integer PRIMARY_DELAY_TAPS =
		PRIMARY_H_DELAY - COLOR_PIPELINE_DELAY;

	initial begin
		if (PRIMARY_H_DELAY <= COLOR_PIPELINE_DELAY)
			$error("vfb_halo_pipeline PRIMARY_H_DELAY is too short");
	end

	vfb_sega_color delayed_color (
		.clk_sys(clk_sys),
		.ce_pix(ce_pix),
		.canonical_in(delayed_canonical),
		.tone_mapping(tone_mapping_q),
		.canonical_out(),
		.red(delayed_r),
		.green(delayed_g),
		.blue(delayed_b)
	...
	logic [3:0] delayed_timing [0:COLOR_PIPELINE_DELAY-1];
	integer timing_i;
	always_ff @(posedge clk_sys) begin
		if (ce_pix) begin
			delayed_timing[0] <= {
			...
			for (timing_i = 1; timing_i < COLOR_PIPELINE_DELAY;
			     timing_i = timing_i + 1)
				delayed_timing[timing_i] <= delayed_timing[timing_i-1];
		end
	end

	wire [3:0] converted_timing =
		delayed_timing[COLOR_PIPELINE_DELAY-1];

	(* ramstyle = "MLAB" *) logic [27:0] horizontal_delay
		[0:PRIMARY_DELAY_TAPS-1];
	integer delay_i;
	always_ff @(posedge clk_sys) begin
		if (ce_pix) begin
			horizontal_delay[0] <= {
			...
			for (delay_i = 1; delay_i < PRIMARY_DELAY_TAPS;
			     delay_i = delay_i + 1)
	...
	wire [27:0] filter_input = {
		horizontal_delay[PRIMARY_DELAY_TAPS-1][27:24],
		horizontal_delay[
			PRIMARY_DELAY_TAPS-1-PRIMARY_DATA_ADVANCE
		][23:0]
	};
	...
	vfb_final_present final_present (
		...
		.reset(reset),
		.ce_pix(ce_pix),
		.color_space_g08_709(color_space_g08_709_q),
