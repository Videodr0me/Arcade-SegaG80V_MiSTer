...
//
// Stores 8x8 tiles with six-bit color, three-bit draw phase, and seven-bit
// intensity.
...
module vfb_tile_cache_manager #(
	...
	// Per-framebuffer tile dirty maps support up to 184 x 135 tiles.
	...
	// Blend crossings in native per-gun energy, then normalize back to one
	// shared intensity and three two-bit color coefficients.
	wire [5:0] old_color = vfb_pixel_color(s2_cached_pixel);
	wire [5:0] new_color = vfb_pixel_color(s1_pixel_data);
	wire [6:0] old_intensity = vfb_pixel_intensity(s2_cached_pixel);
	wire [6:0] new_intensity = vfb_pixel_intensity(s1_pixel_data);

	wire [8:0] old_r_energy =
		vfb_channel_energy(old_color[5:4], old_intensity);
	wire [8:0] old_g_energy =
		vfb_channel_energy(old_color[3:2], old_intensity);
	wire [8:0] old_b_energy =
		vfb_channel_energy(old_color[1:0], old_intensity);
	wire [8:0] new_r_energy =
		vfb_channel_energy(new_color[5:4], new_intensity);
	wire [8:0] new_g_energy =
		vfb_channel_energy(new_color[3:2], new_intensity);
	wire [8:0] new_b_energy =
		vfb_channel_energy(new_color[1:0], new_intensity);

	wire [8:0] crossed_r_energy =
		vfb_soft_cross_energy(old_r_energy, new_r_energy);
	wire [8:0] crossed_g_energy =
		vfb_soft_cross_energy(old_g_energy, new_g_energy);
	wire [8:0] crossed_b_energy =
		vfb_soft_cross_energy(old_b_energy, new_b_energy);
	...
	// Write all bytes; bitmap-masked pixels are zero.
	...
	always_ff @(posedge clk_sys) begin
		...
		case (rmw_state)
				...
				RMW_WAIT_FILL_FINISH: begin
					// Let the final fill write settle before reading it back.
					if (!cache_wr_en[s1_alloc_idx])
						rmw_state <= RMW_READ;
				end
