...
module vfb_halo_wide #(
	...
	localparam [11:0] RECON_DELAY = 12'(RECONSTRUCTION_DELAY_LINES);
	...
	wire reconstruction_uses_previous_epoch =
		(source_full_line_for_line < RECON_DELAY);
	...
	wire [11:0] tail_service_line_reconstruction_line =
		(source_full_line_y < RECON_DELAY)
	...
	// Half-pixel phases center the even reduction grid and filter kernel.
	...
	logic signed [14:0] latest_ramp_r;
	logic signed [14:0] latest_ramp_g;
	logic signed [14:0] latest_ramp_b;
	logic signed [14:0] previous_ramp_r;
	logic signed [14:0] previous_ramp_g;
	logic signed [14:0] previous_ramp_b;
	...
	logic signed [14:0] segment_latest_start_r;
	logic signed [14:0] segment_latest_start_g;
	logic signed [14:0] segment_latest_start_b;
	logic signed [14:0] segment_previous_start_r;
	logic signed [14:0] segment_previous_start_g;
	logic signed [14:0] segment_previous_start_b;

	function automatic signed [14:0] interp_start_from_delta(
		input logic [7:0] left,
		input logic signed [8:0] delta
	);
		begin
			interp_start_from_delta =
				$signed({1'b0, left, 5'b00000}) +
				{{6{delta[8]}}, delta} +
				15'sd16;
		end
	endfunction

	function automatic signed [14:0] interp_advance(
		input logic signed [14:0] ramp,
		input logic signed [8:0] delta
	);
		begin
			interp_advance = ramp + {{5{delta[8]}}, delta, 1'b0};
		end
	endfunction
	...
	function automatic signed [14:0] vertical_lerp_base(
		...
		begin
			vertical_lerp_base = $signed({2'b00, left, 5'b00000});
		end
	endfunction
	...
	function automatic [7:0] vertical_lerp_finish(
		...
		begin
			rounded = base + scaled_delta + 15'sd16;
			vertical_lerp_finish = rounded[12:5];
		end
	endfunction
	...
	always_ff @(posedge clk_sys) begin
		if (reset) begin
			...
			previous_frame_total_lines <= active_height_q;
			...
			latest_ramp_r <= 15'sd16;
			latest_ramp_g <= 15'sd16;
			latest_ramp_b <= 15'sd16;
			previous_ramp_r <= 15'sd16;
			previous_ramp_g <= 15'sd16;
			previous_ramp_b <= 15'sd16;
			...
			segment_latest_start_r <= 15'sd16;
			segment_latest_start_g <= 15'sd16;
			segment_latest_start_b <= 15'sd16;
			segment_previous_start_r <= 15'sd16;
			segment_previous_start_g <= 15'sd16;
			segment_previous_start_b <= 15'sd16;
		end else begin
			segment_read_valid <= 1'b0;
