...
//
// A completed raw frame is composed with the newest accumulator.
...
module vfb_phosphor_compositor #(
	...
	logic [6:0] raw_intensity_q;
	logic [7:0] raw_factor_q;
	logic       raw_bypass_q;
	logic [14:0] raw_product_q;
	logic [5:0] old_color_q;
	logic [6:0] old_intensity_q;
	logic [7:0] old_factor_q;
	logic [14:0] old_product_q;
	...
	wire [2:0] staged_raw_age_delta =
		reference_draw_idx_q - vfb_raw_draw_idx(next_raw_pixel_q);
	...
	wire [6:0] raw_intensity = raw_bypass_q
		? raw_intensity_q : raw_product_q[14:8];
	wire [6:0] old_intensity = old_product_q[14:8];
	wire [8:0] raw_r_energy =
		vfb_channel_energy(raw_color_q[5:4], raw_intensity);
	wire [8:0] raw_g_energy =
		vfb_channel_energy(raw_color_q[3:2], raw_intensity);
	wire [8:0] raw_b_energy =
		vfb_channel_energy(raw_color_q[1:0], raw_intensity);
	wire [8:0] old_r_energy =
		vfb_channel_energy(old_color_q[5:4], old_intensity);
	wire [8:0] old_g_energy =
		vfb_channel_energy(old_color_q[3:2], old_intensity);
	wire [8:0] old_b_energy =
		vfb_channel_energy(old_color_q[1:0], old_intensity);
	wire [8:0] raw_peak =
	...
	always_ff @(posedge clk_sys) begin
		if (reset) begin
			...
			raw_color_q <= 6'd0;
			raw_intensity_q <= 7'd0;
			raw_factor_q <= 8'd0;
			raw_bypass_q <= 1'b0;
			raw_product_q <= 15'd0;
			old_color_q <= 6'd0;
			old_intensity_q <= 7'd0;
			old_factor_q <= 8'd0;
			old_product_q <= 15'd0;
			...
			case (state)
				...
				COMP_PIXEL_LOAD: begin
					raw_color_q <= vfb_pixel_color(next_raw_pixel_q);
					raw_intensity_q <= vfb_pixel_intensity(next_raw_pixel_q);
					raw_factor_q <= staged_raw_factor;
					raw_bypass_q <= (intra_mode_q == 2'd0);
					old_color_q <= vfb_pixel_color(next_old_pixel_q);
					old_intensity_q <= vfb_pixel_intensity(next_old_pixel_q);
					old_factor_q <= (!source_is_composed_q ||
						vfb_composed_fresh(next_old_pixel_q))
						? fresh_decay_factor_q : tail_decay_factor_q;
					state <= COMP_PIXEL_DECAY;
				end

				COMP_PIXEL_DECAY: begin
					...
					state <= COMP_PIXEL_ENERGY;
				end

				COMP_PIXEL_ENERGY: begin
					raw_peak_q <= raw_peak;
					old_peak_q <= old_peak;
					if (pixel_index != 6'd63) begin
						next_raw_factor_q <= staged_raw_factor;
						next_old_factor_q <=
							(!source_is_composed_q ||
							 vfb_composed_fresh(next_old_pixel_q))
							? fresh_decay_factor_q : tail_decay_factor_q;
					end
					state <= COMP_PIXEL_SELECT;
				end
