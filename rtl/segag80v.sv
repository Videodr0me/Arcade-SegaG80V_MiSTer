//============================================================================
//  Sega G-80 X-Y machine: CPU board + EPROM board + X-Y boards
//
//  Clocking
//    One 15.46848 MHz master domain with hardware-ratio enables:
//        Z80   15468480 / 4 = 3.86712 MHz
//        VCL   15468480 / 6 = 2.57808 MHz   (vector generator steps)
//
//  Frame cadence
//    Sega restarts the vector list once per 40 Hz EDGINT with DRAW asserted
//
//  This program is free software under the GNU General Public License v3.
//============================================================================

`default_nettype none

module segag80v #(
	parameter int PHASE_CLKS  = 16,
	parameter int WAIT_STATES = 2,
	parameter int CLK_HZ      = 15_468_480,
	parameter bit SPEECH_FILTER = 1'b1,
	parameter bit SPEECH_C10_TENTH = 1'b0
) (
	input  wire        clk_master,
	input  wire        reset,
	input  wire        pause,

	// ---- MRA configuration ----
	input  wire  [2:0] cfg_chip,     // security chip, see sega_security_pkg
	input  wire        cfg_usb,      // Universal Sound Board RAM at $D000
	input  wire        cfg_speech,   // speech board fitted
	input  wire  [2:0] cfg_game,     // sega_game_pkg id, for the discrete board
	input  wire  [1:0] cfg_fc,       // 0 = plain, 1 = spinner, 2 = elim4

	// ---- packed game and sound ROM download; layout below ----
	input  wire        rom_wr,
	input  wire [16:0] rom_addr,
	input  wire  [7:0] rom_data,

	// ---- controls ----
	input  wire  [7:0] in_d7d6,
	input  wire  [7:0] in_d5d4,
	input  wire  [7:0] in_d3d2,
	input  wire  [7:0] in_d1d0,
	input  wire  [7:0] in_fc,
	input  wire  [7:0] in_coins,
	input  wire  [8:0] spin_magnitude,
	input  wire        spin_direction,
	input  wire        spin_stb,
	input  wire        coin_a,
	input  wire        coin_b,
	input  wire        service,
	input  wire        self_test,   // toggles once for each TEST-button press

	// ---- beam output ----
	output wire signed [12:0] vec_x,
	output wire signed [12:0] vec_y,
	output wire  [5:0] vec_colour,
	output wire        vec_beam,
	output wire        vec_valid,
	output wire        drawing,
	output wire        frame_done,
	output wire        frame_start,
	output wire        vec_tick,    // VCL step enable

	// ---- sound board strobes ----
	output wire        snd_wr,
	output wire  [1:0] snd_sel,
	output wire        ay_wr,
	output wire        ay_port,
	output wire        speech_data_wr,
	output wire        speech_ctrl_wr,
	output wire        usb_data_wr,
	output wire  [7:0] snd_data,

	// ---- audio ----
	output wire signed [15:0] audio_ay,   // raw board outputs for diagnostics
	output wire signed [15:0] audio_speech,
	output wire signed [15:0] audio_usb,
	output wire signed [15:0] audio_discrete,
	output wire signed [15:0] audio_mix,  // cabinet-routed output

	// Simulation taps for USB, speech, and interrupt behavior.
	output wire        dbg_usb_tick,
	output wire        dbg_usb_noise,
	output wire  [8:0] dbg_usb_tmr,
	output wire  [2:0] dbg_usb_cfg,
	output wire [71:0] dbg_usb_env,
	output wire [10:0] dbg_sp_prog_addr,
	output wire        dbg_sp_wr,
	output wire  [7:0] dbg_sp_data,
	output wire        dbg_sp_drq,
	output wire        dbg_sp_t0,
	output wire  [7:0] dbg_sp_p1,
	output wire        dbg_sp_rd_n,
	output wire [13:0] dbg_sp_data_addr,
	output wire        dbg_sp_int_n,
	output wire signed [7:0] dbg_sp_dac,
	output wire  [1:0] coin_counter,
	output wire        dbg_irq,
	output wire  [1:0] dbg_coin_ff,
	output wire        dbg_int_ack
);
	import sega_game_pkg::*;

	wire has_discrete = (cfg_game == GAME_ELIM2) ||
	                    (cfg_game == GAME_ELIM2C) ||
	                    (cfg_game == GAME_ELIM4) ||
	                    (cfg_game == GAME_SPACFURY) ||
	                    (cfg_game == GAME_ZEKTOR);
	wire has_ay = (cfg_game == GAME_ZEKTOR);

	// ------------------------------------------------------------------
	// Machine timing divided from the 15.46848 MHz master
	// ------------------------------------------------------------------
	wire ce_cpu, ce_vcl, ce_vcl_2x, ce_ay, edgint;

	sega_machine_timing timing (
		.clk     (clk_master),
		.reset   (reset),
		.pause   (pause),
		.ce_cpu  (ce_cpu),
		.ce_vcl  (ce_vcl),
		.ce_vcl_2x(ce_vcl_2x),
		.ce_ay   (ce_ay),
		.edgint  (edgint)
	);

	// ------------------------------------------------------------------
	// Program ROM, $0000-$BFFF (48K) in block RAM
	// ------------------------------------------------------------------
	// ROM image layout, shared with sim/tools/build_rom.py and make_mra.py:
	//   $00000  48K program
	//   $0C000   1K sin/cos PROM
	//   $0C400   2K speech board 8035 program
	//   $0CC00  16K speech board LPC data
	localparam [16:0] PROM_BASE   = 17'h0C000;
	localparam [16:0] SPEECH_BASE = 17'h0C400;

	wire [15:0] cpu_rom_addr;
	logic [7:0] cpu_rom_data;

	(* ramstyle = "M10K" *) logic [7:0] prog_rom [0:49151];

	wire rom_wr_prog   = rom_wr && (rom_addr < PROM_BASE);
	wire rom_wr_sin    = rom_wr && (rom_addr >= PROM_BASE)
	                            && (rom_addr < PROM_BASE + 17'd1024);
	wire rom_wr_speech = rom_wr && (rom_addr >= SPEECH_BASE);

	always_ff @(posedge clk_master) begin
		if (rom_wr_prog) prog_rom[rom_addr[15:0]] <= rom_data;
		cpu_rom_data <= prog_rom[cpu_rom_addr];
	end

	// ------------------------------------------------------------------
	// CPU board
	// ------------------------------------------------------------------
	wire [11:0] vram_addr, usb_addr;
	wire  [7:0] vram_din,  usb_din;
	wire        vram_wr,   usb_wr;
	wire  [7:0] vram_dout, usb_dout, usb_status;
	wire        snd_wr_raw, ay_wr_raw;
	wire        speech_data_wr_raw, speech_ctrl_wr_raw, usb_data_wr_raw;

	segag80v_cpu #(.WAIT_STATES(WAIT_STATES)) cpu (
		.clk        (clk_master),
		.ce_cpu     (ce_cpu),
		.reset      (reset),
		.cfg_chip   (cfg_chip),
		.cfg_usb    (cfg_usb),
		.cfg_fc     (cfg_fc),
		.rom_addr   (cpu_rom_addr),
		.rom_data   (cpu_rom_data),
		.vram_addr  (vram_addr),
		.vram_din   (vram_din),
		.vram_wr    (vram_wr),
		.vram_dout  (vram_dout),
		.usb_addr   (usb_addr),
		.usb_din    (usb_din),
		.usb_wr     (usb_wr),
		.usb_dout   (usb_dout),
		.in_d7d6    (in_d7d6),
		.in_d5d4    (in_d5d4),
		.in_d3d2    (in_d3d2),
		.in_d1d0    (in_d1d0),
		.in_fc      (in_fc),
		.in_coins   (in_coins),
		.spin_magnitude (spin_magnitude),
		.spin_direction (spin_direction),
		.spin_stb       (spin_stb),
		.draw_flag  (drawing),
		.edgint     (edgint),
		.coin_a     (coin_a),
		.coin_b     (coin_b),
		.service    (service),
		.self_test  (self_test),
		.snd_wr     (snd_wr_raw),
		.snd_sel    (snd_sel),
		.ay_wr      (ay_wr_raw),
		.ay_port    (ay_port),
		.speech_data_wr (speech_data_wr_raw),
		.speech_ctrl_wr (speech_ctrl_wr_raw),
		.usb_data_wr    (usb_data_wr_raw),
		.usb_status     (usb_status),
		.io_dout    (),
		.coin_counter (coin_counter),
		.dbg_wram_wr      (),
		.dbg_wram_addr_raw(),
		.dbg_wram_addr_scr(),
		.dbg_wram_data    (),
		.dbg_io_rd        (),
		.dbg_port         (),
		.dbg_irq(dbg_irq), .dbg_coin_ff(dbg_coin_ff), .dbg_int_ack(dbg_int_ack),
		.dbg_op_addr  ()
	);

	assign snd_wr         = has_discrete && snd_wr_raw;
	assign ay_wr          = has_ay       && ay_wr_raw;
	assign speech_data_wr = cfg_speech   && speech_data_wr_raw;
	assign speech_ctrl_wr = cfg_speech   && speech_ctrl_wr_raw;
	assign usb_data_wr    = cfg_usb      && usb_data_wr_raw;

	// Expose active-list and EOF timing in VCL ticks.
	assign vec_tick = ce_vcl;
	assign frame_start = edgint;

	// All sound-board write strobes use the CPU data bus.
	assign snd_data = usb_din;

	wire signed [15:0] usb_audio;
	wire signed [15:0] speech_aux;

	sega_audio_router audio_router (
		.game           (cfg_game),
		.pause          (pause),
		.ay_audio       (audio_ay),
		.speech_audio   (audio_speech),
		.usb_audio      (usb_audio),
		.discrete_audio (audio_discrete),
		.speech_aux     (speech_aux),
		.audio          (audio_mix)
	);

	// ------------------------------------------------------------------
	// Speech board (Space Fury, Zektor, Star Trek)
	// ------------------------------------------------------------------
	sega_speech #(.CLK_HZ(CLK_HZ), .FILTER(SPEECH_FILTER),
	              .C10_TENTH(SPEECH_C10_TENTH)) speech (
		.clk       (clk_master),
		.reset     (reset || !cfg_speech),
		.data_wr   (speech_data_stb),
		.ctrl_wr   (speech_ctrl_stb),
		.din       (snd_data),
		.rom_wr    (rom_wr_speech),
		.rom_addr  (rom_addr[14:0] - SPEECH_BASE[14:0]),
		.rom_data  (rom_data),
		.aux_audio (speech_aux),
		.audio     (audio_speech),
		.dbg_prog_addr(dbg_sp_prog_addr),
		.dbg_sp_wr    (dbg_sp_wr),
		.dbg_sp_data  (dbg_sp_data),
		.dbg_drq      (dbg_sp_drq),
		.dbg_t0       (dbg_sp_t0),
		.dbg_p1       (dbg_sp_p1),
		.dbg_rd_n     (dbg_sp_rd_n),
		.dbg_data_addr(dbg_sp_data_addr),
		.dbg_int_n    (dbg_sp_int_n),
		.dbg_dac      (dbg_sp_dac)
	);

	// ------------------------------------------------------------------
	// Universal Sound Board (Tac/Scan, Star Trek)
	// ------------------------------------------------------------------
	sega_usb #(.CLK_HZ(CLK_HZ)) usb (
		.clk      (clk_master),
		.reset    (reset || !cfg_usb),
		.data_wr  (usb_data_stb),
		.din      (snd_data),
		.status   (usb_status),
		.pgm_addr (usb_addr),
		.pgm_din  (usb_din),
		.pgm_wr   (usb_wr),
		.pgm_dout (usb_dout),
		.audio    (usb_audio),
		.dbg_tick (dbg_usb_tick),
		.dbg_noise(dbg_usb_noise),
		.dbg_tmr  (dbg_usb_tmr),
		.dbg_cfg  (dbg_usb_cfg),
		.dbg_env  (dbg_usb_env)
	);

	assign audio_usb = usb_audio;

	// $38 and $3B are levels for the whole I/O cycle; strobe once at the end.
	logic sp_data_d, sp_ctrl_d, usb_data_d;
	always_ff @(posedge clk_master) begin
		if (reset) begin
			sp_data_d  <= 1'b0;
			sp_ctrl_d  <= 1'b0;
			usb_data_d <= 1'b0;
		end else begin
			sp_data_d  <= speech_data_wr;
			sp_ctrl_d  <= speech_ctrl_wr;
			usb_data_d <= usb_data_wr;
		end
	end
	wire speech_data_stb = ~speech_data_wr && sp_data_d;

	// ------------------------------------------------------------------
	// Discrete sound board (Eliminator, Zektor, Space Fury)
	// ------------------------------------------------------------------
	logic snd_wr_d;
	always_ff @(posedge clk_master) begin
		if (reset) snd_wr_d <= 1'b0;
		else       snd_wr_d <= snd_wr;
	end

	sega_discrete #(.CLK_HZ(CLK_HZ)) disc (
		.clk   (clk_master),
		.reset (reset || !has_discrete),
		.game  (cfg_game),
		.wr    (~snd_wr && snd_wr_d),
		.sel   (snd_sel[0]),
		.din   (snd_data),
		.audio (audio_discrete)
	);
	wire speech_ctrl_stb = ~speech_ctrl_wr && sp_ctrl_d;
	wire usb_data_stb    = ~usb_data_wr    && usb_data_d;

	// Only Zektor enables the AY-3-8912 interface at $3C/$3D.
	sega_ay ay (
		.clk      (clk_master),
		.ce       (ce_ay),
		.reset    (reset || !has_ay),
		.wr       (ay_wr_stb),
		.addr_sel (ay_port_sel),
		.din      (snd_data),
		.audio    (audio_ay)
	);

	// Convert the full-cycle AY write level into one end-of-cycle strobe.
	logic ay_wr_d;
	logic ay_port_sel;
	always_ff @(posedge clk_master) begin
		if (reset) begin
			ay_wr_d     <= 1'b0;
			ay_port_sel <= 1'b0;
		end else begin
			ay_wr_d <= ay_wr;
			if (ay_wr) ay_port_sel <= ay_port;
		end
	end
	wire ay_wr_stb = ~ay_wr && ay_wr_d;

	// ------------------------------------------------------------------
	// X-Y boards
	// ------------------------------------------------------------------
	wire        segment_start;
	wire [11:0] segment_start_x, segment_start_y;
	wire  [8:0] segment_duration;
	wire  [7:0] segment_delta_x, segment_delta_y;
	wire        segment_x_negative, segment_y_negative;
	wire  [5:0] segment_colour;
	wire        segment_beam, segment_dot;

	sega_xy_top #(
		.PHASE_CLKS (PHASE_CLKS)
	) xy (
		.clk         (clk_master),
		.ce          (ce_vcl),
		.reset       (reset),
		.frame_start (edgint),
		.cpu_addr    (vram_addr),
		.cpu_din     (vram_din),
		.cpu_wr      (vram_wr),
		.cpu_dout    (vram_dout),
		.rom_wr      (rom_wr_sin),
		.rom_addr    (rom_addr[9:0]),
		.rom_data    (rom_data),
		.out_x       (),
		.out_y       (),
		.out_colour  (),
		.out_beam    (),
		.out_valid   (),
		.segment_start          (segment_start),
		.segment_start_x        (segment_start_x),
		.segment_start_y        (segment_start_y),
		.segment_duration       (segment_duration),
		.segment_delta_x        (segment_delta_x),
		.segment_delta_y        (segment_delta_y),
		.segment_x_negative     (segment_x_negative),
		.segment_y_negative     (segment_y_negative),
		.segment_colour         (segment_colour),
		.segment_beam           (segment_beam),
		.segment_dot            (segment_dot),
		.drawing     (drawing),
		.frame_done  (frame_done)
	);

	// The presentation DDA doubles spatial density without changing the
	// authentic sequencer, endpoint calculations, or frame cadence.
	sega_xy_shadow shadow (
		.clk                    (clk_master),
		.reset                  (reset),
		.frame_start            (edgint),
		.ce_2x                  (ce_vcl_2x),
		.segment_start          (segment_start),
		.segment_start_x        (segment_start_x),
		.segment_start_y        (segment_start_y),
		.segment_duration       (segment_duration),
		.segment_delta_x        (segment_delta_x),
		.segment_delta_y        (segment_delta_y),
		.segment_x_negative     (segment_x_negative),
		.segment_y_negative     (segment_y_negative),
		.segment_colour         (segment_colour),
		.segment_beam           (segment_beam),
		.segment_dot            (segment_dot),
		.out_x                  (vec_x),
		.out_y                  (vec_y),
		.out_colour             (vec_colour),
		.out_beam               (vec_beam),
		.out_valid              (vec_valid),
		.busy                   ()
	);

endmodule

`default_nettype wire
