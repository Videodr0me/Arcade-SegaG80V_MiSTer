...
module segag80v_cpu #(
	...
	input  wire        coin_b,
	input  wire        service,     // active-low service-credit switch, P1.10
	input  wire        self_test,   // event toggle from CPU-board TEST control
	...
	//
	// Port $BF writes have no observed effect in the supported games.
	// ------------------------------------------------------------------
	logic [1:0] coin_ff;
	logic       edgint_ff;
	logic       irq_line;
	...
	// One toggle represents one press of the momentary CPU-board TEST switch.
	logic self_test_q;
	assign nmi_n = ~(self_test ^ self_test_q);

	always_ff @(posedge clk) begin
		if (reset)
			self_test_q <= self_test;
		else if (ce_cpu)
			self_test_q <= self_test;
	end
