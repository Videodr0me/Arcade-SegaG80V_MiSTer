...
# Clock-domain crossings use synchronizers, stable-data mailboxes, or the
# renderer's asynchronous source FIFO.
set emu_clk_125 [get_clocks {emu|pll|pll_inst|altera_pll_i|general[0].gpll~PLL_OUTPUT_COUNTER|divclk}]
set emu_clk_master [get_clocks {emu|machine_clocks|vec_pll|general[0].gpll~PLL_OUTPUT_COUNTER|divclk}]
...
set_clock_groups -asynchronous -group $sys_clk_50 -group $emu_clk_master
set_clock_groups -asynchronous -group $sys_clk_audio -group $emu_clk_master
set_clock_groups -asynchronous -group $sys_clk_hdmi -group $emu_clk_master
